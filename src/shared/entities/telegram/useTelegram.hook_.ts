...
export function useTelegram() {
  ...
  // const [chatsRequested, setChatsRequested] = useState<number[]>([]);

  // function request(cs: TChat[]) {
  //   setChatsRequested(s => [...s, ...cs.map(c => c.id)]);

  // }

  // useEffect(() => {
  //   request(chats.filter(c => !chatsRequested.includes(c.id)));
  // }, [chats]);
  ...
  //   // while (i !== -1) {
  //   //   // eslint-disable-next-line no-console
  //   //   console.log('>', limit, i, R.last(ids));
  ...
  //   //   else if (maybeChats.payload.total_count - ids.length < 20) limit = maybeChats.payload.total_count - ids.length;
  //   // }
  ...
  const fetchChatsss = useCallback(
    async (chatIds: number[]) => {
      ...
      for await (const chat_id of chatIds) {
        const maybeChat = await TelegramService.send<TChat>({
          type: 'getChat',
          chat_id,
        });
  ...
  const fetchChats = useCallback(async () => {
    ...
    if (maybeChats.isNone) return;

    R.compose(dispatch, setChatIds)(maybeChats.payload.chat_ids);
    fetchChatsss(maybeChats.payload.chat_ids);
    // TelegramService.send({
    //   type: 'loadChats',
    //   limit: 100000,
    // });
  ...
  }, [isRestored, isInitialized, dispatch, fetchChats]);
